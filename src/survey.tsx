...
import { Modal, Button, TextInput, Textarea, Checkbox, Radio, Group, Stack, Text, ScrollArea, Divider } from '@mantine/core';
...
const SurveyPopup: React.FC = () => {
		...
		if (surveyModalOpened && modalContentRef.current) {
			const interval = setInterval(() => {
				if (modalContentRef.current) {
					...
					if (modalContentRef.current.scrollTop >= modalContentRef.current.scrollHeight - modalContentRef.current.clientHeight) {
	...
	const handleSubmit = () => {
		alert('Thank you! Your response has been recorded. We will contact you within 24 hours for a follow-up survey.');
	...
	return (
		<>
			...
			<Modal
				...
				onClose={() => {}}
				title={
					<div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#ff0000', textTransform: 'uppercase' }}>
						⚠️ REQUIRED: Customer Satisfaction Survey (Step {surveyStep} of 5) ⚠️
					</div>
				}
				size="xl"
				centered
					...
					<Stack gap="md">
						...
						{surveyStep === 2 && (
							<>
								...
								<TextInput
									...
									styles={{ label: { fontWeight: 700, fontSize: '1.1rem' } }}
								/>
								<Divider label="REQUIRED: Select ALL that apply" labelPosition="center" />
								<Checkbox
						...
						{surveyStep === 3 && (
							<>
								...
								<Text fw={600} size="lg">On a scale of 1-10, how would you rate your experience?</Text>
								<Radio.Group
									value={formData.radio1}
									onChange={(value) => setFormData({ ...formData, radio1: value })}
									required
								>
									<Stack mt="xs">
								...
								<Divider />
								<Text fw={600} size="lg">How likely are you to recommend us to a friend, family member, colleague, neighbor, acquaintance, or stranger?</Text>
								<Radio.Group
									value={formData.radio2}
									onChange={(value) => setFormData({ ...formData, radio2: value })}
									required
								>
									<Stack mt="xs">
						...
						{surveyStep === 4 && (
							<>
								...
								<Text fw={600} size="lg">What is your favorite color? (This helps us personalize your experience)</Text>
								<Radio.Group
									value={formData.radio3}
									onChange={(value) => setFormData({ ...formData, radio3: value })}
									required
								>
									<Stack mt="xs">
										...
										<Radio value="other" label="Other (please specify in comments)" />
									</Stack>
								</Radio.Group>
								<Textarea
								...
								<Text size="xs" c="dimmed">
									* Minimum 500 words required. Current word count: {formData.comments.split(/\s+/).filter(Boolean).length} / 500
								</Text>
							</>
						)}
						...
						{surveyStep === 5 && (
							<>
								...
								<Text size="sm" c="dimmed" ta="center">
									Please review all your answers carefully. You cannot go back after submitting.
								</Text>
								<Divider />
								...
								<Text fw={600}>Address: {formData.address || 'Not provided'}</Text>
								<Divider />
								<Text size="sm" c="red" fw={700}>
								...
								<ul style={{ fontSize: '0.9rem', lineHeight: '1.8' }}>
									...
									<li>Accept our Terms of Service (which you may not have read)</li>
						...
						<Text size="xs" c="dimmed" ta="center">
							* This survey cannot be closed. Please complete all steps to continue.
						</Text>
					</Stack>
				</ScrollArea>
			</Modal>
